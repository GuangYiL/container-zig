...
`zig-0.15` 分支面向 Zig `0.15.2`，公开导入模块名为 `container_zig`，并把 Docker Engine API v1.55 端点映射为明确的 Zig 模块函数，例如 `docker.system.ping`、`docker.container.list` 和 `docker.image.build`。
...
- HTTP、TLS、socket、进程和 WebSocket framing 均使用 Zig `0.15.2` 标准库。SSH 传输直接执行用户的 `ssh`，不会经过 shell。
...
`main` 分支面向 Zig `0.16.0`；`zig-0.17` 跟踪下一个开发快照。
...
`Client.init(allocator, config)` 返回 error union，并校验选中的传输。所有带版本的端点调用前必须执行 `try client.connect()`。自动版本和固定版本都会校验 daemon 支持的 API 区间。
...
- `Client.init` 显式接收 allocator；会分配内存的端点函数把 allocator 放在首位。
...
- `zig-0.15` 分支使用 Zig `0.15.2` 验证，不包含 Zig 0.16 兼容 shim。
