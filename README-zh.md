...
`main` 分支面向 Zig `0.16.0`，公开导入模块名为 `container_zig`，并把 Docker Engine API v1.55 端点映射为明确的 Zig 模块函数，例如 `docker.system.ping`、`docker.container.list` 和 `docker.image.build`。
...
- HTTP、TLS、socket、进程和 WebSocket framing 均使用 Zig `0.16` 标准库。SSH 传输直接执行用户的 `ssh`，不会经过 shell。
...
`zig-0.15` 分支面向 Zig `0.15.2`；`zig-0.17` 跟踪文档中声明的 Zig `0.17` 开发快照。
...
- `main` 分支使用 Zig `0.16.0` 验证；`zig-0.17` 单独跟踪开发快照，不保留兼容 shim。
