...
pub const Exchange = struct {
    ...
    pub fn create(
        ...
        const connection = try transport_connection.connect(allocator, spec.transport);
...
const TestConnection = struct {
    ...
    fn connect(context: *anyopaque, _: std.mem.Allocator) !@import("transport.zig").Transport.Connection {
...
test "raw transport streams chunked requests and parses responses" {
    ...
    const exchange = try Exchange.create(std.testing.allocator, .{
        .method = .post,
        .path = "/v1.55/test",
