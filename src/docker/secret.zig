...
pub const SecretList = struct {
    ...
    pub fn deinit(self: *SecretList) void {
...
pub fn list(allocator: std.mem.Allocator, client: *Client, options: ListOptions) !SecretList {
...
pub fn create(allocator: std.mem.Allocator, client: *Client, options: CreateOptions) !Create {
    ...
    switch (response.status()) {
        .created => {},
        .conflict => return error.SecretNameConflict,
...
pub fn inspect(allocator: std.mem.Allocator, client: *Client, id: []const u8) !Secret {
    const path = try url.pathWithSegment(allocator, "/secrets/", id, "");
    defer allocator.free(path);
    var response = try client.request(.{ .method = .get, .path = path });
    defer response.deinit();
    try expectSecret(response.status());
    const body = try response.body() orelse return error.EmptyResponse;
    return parseSecret(allocator, body);
}
...
pub fn remove(allocator: std.mem.Allocator, client: *Client, id: []const u8) !void {
    ...
    defer response.deinit();
    try expectNoContentSecret(response.status());
}
...
pub fn update(allocator: std.mem.Allocator, client: *Client, id: []const u8, options: UpdateOptions) !void {
    ...
    switch (response.status()) {
        ...
        .not_found => return error.SecretNotFound,
        .internal_server_error => return error.ServerError,
        .service_unavailable => return error.NotInSwarm,
        else => return error.UnexpectedStatus,
    }
...
const RawSecret = struct {
    ID: []const u8,
    Version: ?RawVersion = null,
    CreatedAt: ?[]const u8 = null,
    UpdatedAt: ?[]const u8 = null,
    Spec: ?RawSecretSpec = null,
};

const RawSecretSpec = struct { Name: ?[]const u8 = null };
const RawVersion = struct { Index: ?u64 = null };
const RawId = struct { ID: []const u8 };

fn parseList(allocator: std.mem.Allocator, body: []const u8) !SecretList {
    const parsed = try std.json.parseFromSlice([]RawSecret, allocator, body, json.owned_parse_options);
    defer parsed.deinit();
    const items = try allocator.alloc(Secret, parsed.value.len);
    var filled: usize = 0;
    errdefer {
    ...
    for (parsed.value) |raw| {
        items[filled] = try secretFromRaw(allocator, raw);
        filled += 1;
    }
    return .{ .allocator = allocator, .items = items };
}

fn parseSecret(allocator: std.mem.Allocator, body: []const u8) !Secret {
    const parsed = try std.json.parseFromSlice(RawSecret, allocator, body, json.owned_parse_options);
    defer parsed.deinit();
    return secretFromRaw(allocator, parsed.value);
}
...
fn secretFromRaw(allocator: std.mem.Allocator, raw: RawSecret) !Secret {
...
fn listPath(allocator: std.mem.Allocator, options: ListOptions) ![]u8 {
    var builder = try query.Builder.init(allocator, "/secrets");
...
fn updatePath(allocator: std.mem.Allocator, id: []const u8, version: i64) ![]u8 {
    const base_path = try url.pathWithSegment(allocator, "/secrets/", id, "/update");
...
fn expectSecret(status: http.Status) !void {
    switch (status) {
        .ok => {},
        .not_found => return error.SecretNotFound,
        .internal_server_error => return error.ServerError,
...
fn expectNoContentSecret(status: http.Status) !void {
    switch (status) {
        .no_content => {},
        .not_found => return error.SecretNotFound,
        .internal_server_error => return error.ServerError,
...
test "secret paths body and parsing" {
    const list_path = try listPath(std.testing.allocator, .{ .filters = "{\"name\":[\"db\"]}" });
    defer std.testing.allocator.free(list_path);
    try std.testing.expectEqualStrings("/secrets?filters=%7B%22name%22%3A%5B%22db%22%5D%7D", list_path);

    const update_path_result = try updatePath(std.testing.allocator, "secret/id", 4);
    defer std.testing.allocator.free(update_path_result);
    try std.testing.expectEqualStrings("/secrets/secret%2Fid/update?version=4", update_path_result);

    const body = try json.stringifyAlloc(std.testing.allocator, Spec{
        .name = "db",
        .data = "ZGI=",
        .labels = .{ .entries = &.{.{ .name = "tier", .value = "data" }} },
    });
    defer std.testing.allocator.free(body);
    try std.testing.expect(std.mem.containsAtLeast(u8, body, 1, "\"Data\":\"ZGI=\""));

    var secret = try parseSecret(
        std.testing.allocator,
        "{\"ID\":\"secret-id\",\"Version\":{\"Index\":2},\"Spec\":{\"Name\":\"db\"}}",
    );
    defer secret.deinit(std.testing.allocator);
    try std.testing.expectEqualStrings("db", secret.spec_name.?);
}
