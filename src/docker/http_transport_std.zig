...
pub const Exchange = struct {
    ...
    pub fn create(
        ...
        const supplied_connection = switch (spec.transport) {
            .unix_socket => |config| if (builtin.os.tag == .windows)
                return error.UnixSocketUnsupported
            else
                try exchange.connectUnix(config.path),
            .tcp, .tls => null,
    ...
    fn connectUnix(self: *Exchange, path: []const u8) !*std.http.Client.Connection {
        ...
        // Zig 0.15.2 std.http.Client.connectUnix contains stale Connection fields.
        // Zig 0.15.2 的 std.http.Client.connectUnix 包含已失效的 Connection 字段。
        // Inject a standard-library Unix stream until the upstream function is fixed.
        // 上游函数修复前，注入标准库 Unix stream。
        self.connection = .{
            .client = self.client,
            .stream_writer = stream.writer(self.write_buffer),
            .stream_reader = stream.reader(self.read_buffer),
            .pool_node = .{},
            .port = 0,
            .host_len = 0,
            .proxied = false,
    ...
    pub fn closeWrite(self: *Exchange) !void {
        ...
        if (builtin.os.tag == .windows) return error.HalfCloseUnsupported;
        try std.posix.shutdown(connection.stream_writer.getStream().handle, .send);
    }
