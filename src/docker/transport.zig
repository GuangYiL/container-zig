...
pub const Transport = union(enum) {
    ...
    pub const Custom = struct {
        context: *anyopaque,
        connect_fn: *const fn (*anyopaque, std.mem.Allocator, std.Io) anyerror!Connection,

        pub fn connect(self: Custom, allocator: std.mem.Allocator, io: std.Io) !Connection {
            return self.connect_fn(self.context, allocator, io);
        }
    };
