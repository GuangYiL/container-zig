...
pub const Client = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, io: std.Io, config: Config) !Client {
        ...
        return .{
            .allocator = allocator,
            .http = http_transport.Transport.init(allocator, io),
    ...
    fn retryDelay(self: *Client) !void {
        ...
        if (milliseconds > 0) {
            try std.Io.sleep(self.http.io, .fromMilliseconds(@intCast(milliseconds)), .awake);
        }
    }
