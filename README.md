...
The `zig-0.17` branch targets Zig `0.17.0-dev.1387+01b60634c`, exposes the import module `container_zig`, and maps Docker Engine API v1.55 endpoints to explicit Zig module functions.
...
- HTTP, TLS, sockets, processes, and WebSocket framing use the Zig `0.17` standard library. SSH transport executes the user's `ssh` binary directly without a shell.
...
The `main` branch keeps the equivalent SDK behavior on Zig `0.16.0`; `zig-0.15` targets Zig `0.15.2`.
...
`Client.init(allocator, io, config)` returns an error union and validates the selected transport. Call `try client.connect()` before any versioned endpoint. Automatic and fixed modes both validate the daemon's supported API range.
...
- `Client.init` receives both the allocator and `std.Io`; endpoint functions that allocate place the allocator first.
...
- The `zig-0.17` branch is tested with Zig `0.17.0-dev.1387+01b60634c` and contains no Zig 0.16 compatibility shims.
...
- [Zig development documentation](https://ziglang.org/documentation/master/)
