...
The `main` branch targets Zig `0.16.0`, exposes the import module `container_zig`, and maps Docker Engine API v1.55 endpoints to explicit Zig module functions such as `docker.system.ping`, `docker.container.list`, and `docker.image.build`.
...
- HTTP, TLS, sockets, processes, and WebSocket framing use the Zig `0.16` standard library. SSH transport executes the user's `ssh` binary directly without a shell.
...
The `zig-0.15` branch targets Zig `0.15.2`; `zig-0.17` tracks the documented Zig `0.17` development snapshot.
...
- The `main` branch is tested with Zig `0.16.0`; `zig-0.17` tracks the separate development snapshot without compatibility shims.
