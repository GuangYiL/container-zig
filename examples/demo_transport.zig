...
pub fn resolve(
    ...
    const path = try discoverUnixSocket(arena, io, environment);
...
fn parseHost(allocator: std.mem.Allocator, value: []const u8) !docker.Transport {
    ...
    const host = (try uri.getHostAlloc(allocator)).bytes;
...
fn discoverUnixSocket(
    ...
    if (try isUnixSocket(io, docker.Transport.default_unix_socket_path)) {
    ...
    for (user_socket_suffixes) |suffix| {
        ...
        if (try isUnixSocket(io, candidate)) return candidate;
    }
    return error.DockerSocketNotFound;
}

fn isUnixSocket(io: std.Io, path: []const u8) !bool {
